...
class Mushie {
  ...
  async sync() {
    this.key = this.hdkey.derive(this.path)
    if (this._use) {
      // reinitialize use functions with the new key
      for(let key in this._use) {
        this[key] = this._use[key](this.key)
      }
      if (this._init) {
...
class Maker {
  ...
  async make(o) {
    let mushie = new Mushie(o, this.hdkey)
    await mushie.sync()
    return mushie
  }
  exists (p) {
